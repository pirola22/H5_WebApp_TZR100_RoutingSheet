    ...
    export class AppController {
        ...
        */
        private mapKeyUp(event: any) {
            //F4 : 115, ENTER : 13
            if (event.keyCode === 115 && document.activeElement.name === "finalMo") {
        ...
        */
        ...
        private createRoutingSheet(): void {
            ...
            if (finalMo == "" || facility == "" || structureType == "" || finalProduct == "") {
                ...
                let warningMessage = "Please select a final MO using Prompt(F4).";
                ...
                this.scope.routing.openMos.selectedItems.forEach((MOs: any, index: any) => {
                    ...
                    this.appService.GetMo(facility, itemNo, MONumber).then((val:any) => {
                        this.appService.addFieldValue(
                            ...
                            .then((val: any) => {
                                this.scope.routing.transactionStatus.addFieldValue = false;
                                this.refreshTransactionStatus();
        ...
        private findRoutingSheet(): void {
            ...
            this.appService.LstRoutingSheet(routingSheetNo).then((val:any) => {
                ...
                this.scope.routing.routingSheet.finalProduct = val.items[0].F1A030;
        ...
        private LstRoutingSheet(routingSheetNo: string): void {
            ...
            this.scope.routing.routingSheet.facility = this.scope.routing.finalProductSearch.selectedItem.VHFACI;
            this.scope.routing.routingSheet.finalMO = this.scope.routing.finalProductSearch.selectedItem.VHMFNO;
            this.scope.routing.routingSheet.finalProduct = this.scope.routing.finalProductSearch.selectedItem.VHPRNO;
        ...
        private deleteLines(): void{
            ...
            this.scope.routing.routingSheet.selectedItems.forEach((MOs: any, index: any) => {
                ...
                this.scope.loadingData = true;
                this.scope.routing.transactionStatus.deleteFieldValue = true;
                
                this.appService.DeleteFieldValue(MFNO, routingSheetNo, Seq, WHLO).then((val: any) => {
                    
                    this.scope.routing.transactionStatus.deleteFieldValue = false;
                    this.refreshTransactionStatus();
                }, (err: any) => {
                    let error = "API: " + err.program + "." + err.transaction + ", Error Code: " + err.errorCode;
                    this.showError(error, [err.errorMessage]);
        ...
        public getMediaNode(doc: Document, file: string): any {
            ...
            var PrintNode = doc.createElement("Print");

            var Printer = doc.createElement("Printer");
            Printer.innerHTML = "Floor Printer 23";
            PrintNode.appendChild(Printer);

            var Address = doc.createElement("Address");
            Address.innerHTML = "10.10.20.92";
            PrintNode.appendChild(Address);
            ...
            DocumentType.innerHTML = "PMZ242PF";
